...
pub use async_openai::types::chat::{
    ...
    // `ChatCompletionRequestDeveloperMessage`/`...Args` are redefined locally
    // below to add the Kimi K3 dynamic-tool `tools` field; keep the content type.
    ...
    // `ChatCompletionRequestSystemMessage`/`...Args` are redefined locally below
    // to add the Kimi K3 dynamic-tool `tools` field; keep the content types.
...
/// - `tools`: Moonshot Kimi K3 "dynamically loaded tools" declared inside a
///   system message (`messages[].tools`). Preserved through the typed layer so
///   the K3 renderer can emit an in-place tool-declare block; the internally
///   tagged `ChatCompletionRequestMessage` enum would otherwise silently drop
///   this unknown field during deserialization.
#[derive(Debug, Serialize, Deserialize, Default, Clone, Builder, PartialEq)]
...
pub struct ChatCompletionRequestSystemMessage {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub name: Option<String>,
    /// Kimi K3 dynamic tools declared in this system message. Absent for every
    /// other model/message, so it serializes/deserializes identically to the
    /// upstream shape when omitted.
    #[serde(skip_serializing_if = "Option::is_none", default)]
    pub tools: Option<Vec<ChatCompletionTool>>,
}

/// Developer message with Kimi K3 dynamic-tool support.
///
/// Mirrors [`ChatCompletionRequestSystemMessage`]'s `tools` extension so a
/// developer-role message may also carry `messages[].tools`.
#[derive(Debug, Serialize, Deserialize, Default, Clone, Builder, PartialEq)]
...
pub struct ChatCompletionRequestDeveloperMessage {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub name: Option<String>,
    /// Kimi K3 dynamic tools declared in this developer message. See
    /// [`ChatCompletionRequestSystemMessage::tools`].
    #[serde(skip_serializing_if = "Option::is_none", default)]
    pub tools: Option<Vec<ChatCompletionTool>>,
}
