...
pub trait OAIChatLikeRequest {
    ...
    /// True when the request's top-level `tools` all originated as dynamic
    /// (`messages[].tools`) declarations hoisted to the top level, with no
    /// original global tools. Kimi K3 then renders them under the long
    /// "## New Tools Available" header (Moonshot parity) instead of the short
    /// global "# Tools" header. Default `false` (non-dynamic / other models).
